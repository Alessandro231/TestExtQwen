...
const drawEquippedBow = (ctx, x, y, time, aimAngle = 0, facingRight = true) => {
  const floatY = Math.sin(time / 200) * 2;
  const localAngle = facingRight ? aimAngle : Math.PI - aimAngle;

  ctx.save();
  const anchorX = x + 16;
  const anchorY = y + 16 + floatY;
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.arc(0, 0, 12, -Math.PI * 0.65, Math.PI * 0.65, false);
  ctx.stroke();

  ctx.fillStyle = "#A0522D";
  ctx.beginPath();
  ctx.arc(0, 0, 10, -Math.PI * 0.45, Math.PI * 0.45, false);
  ctx.stroke();
...
const drawArrow = (ctx, arrow) => {
  const x = arrow.x;
  const y = arrow.y;
  const angle = arrow.angle ?? Math.atan2(arrow.vy || 0, arrow.vx || 1);
...
export function renderGameScene(ctx, game, timestamp) {
  ...
  if (Array.isArray(game.checkpoints)) {
    game.checkpoints.forEach((checkpoint) =>
      drawCheckpoint(ctx, checkpoint, timestamp),
    );
  }
  if (portal) {
