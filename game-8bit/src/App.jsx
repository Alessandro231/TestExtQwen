...
function App() {
  ...
  const syncBossHud = useCallback((boss) => {
    const nextHud = { active: boss.active, dead: boss.dead, hp: Math.max(0, boss.hp) }
  ...
  // Control de teclado
  useEffect(() => {
    const isJumpKey = (key) => key === 'ArrowUp' || key === 'w' || key === 'W' || key === ' '
  ...
  return (
    <div className="game-container">
      ...
      <div className="checkpoint-status">
        ...
        <span>{checkpointHud.total > 0 ? `${checkpointHud.activeIndex}/${checkpointHud.total}` : '0/0'}</span>
      </div>
          ...
          <div className="sword-bar">
            ...
            <div className="sword-time-bar">
              <div
                ...
                style={{ width: `${(swordTimeLeft / (SWORD_DURATION / 1000)) * 100}%` }}
              />
            </div>
      ...
      {bossHud.active && !bossHud.dead && (
        <div className="boss-hud">
          ...
          <div className="boss-hp-container">
            ...
            />
            <div className="boss-hp-text">{bossHud.hp} / {BOSS_MAX_HP} HP</div>
          </div>
        </div>
      )}
      ...
      <div className="controls-info">
        <p>⬅️ ➡️ or A/D - Move | ⬆️ or W or SPACE - Jump | SHIFT - Dash | Z - Attack (sword) | C - Shoot (bow) | F3 - Debug</p>
        <p className="sword-hint">Touch a checkpoint flag to set your respawn position in the current level.</p>
      ...
      {debugHud.enabled && (
        <div className="debug-hud">
          ...
          <div>POS: {debugHud.playerX}, {debugHud.playerY}</div>
          <div>VEL: {debugHud.playerVx}, {debugHud.playerVy}</div>
